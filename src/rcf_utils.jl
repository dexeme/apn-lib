...
function matrix_characteristic_polynomial(A::FqMatrix)
    if isdefined(Main, :charpoly)
        return getfield(Main, :charpoly)(A)
    end

    if isdefined(Main, :characteristic_polynomial)
        return getfield(Main, :characteristic_polynomial)(A)
    end

    error("""
    No characteristic polynomial function was found.

    Try in the Julia REPL:

        methods(charpoly)
        methods(characteristic_polynomial)

    Then replace matrix_characteristic_polynomial(A) with the correct function for your Nemo version.
    """)
end
...
function get_rcfs(n::Int)
    ...
    for partition in integer_partitions(n)
        ...
        for blocks in cartesian_product_lists(block_options)
            if valid_rcf_block_sequence(blocks)
                reversed_blocks = FqMatrix[block for block in reverse(blocks)]
                push!(result, block_diagonal_gf2(reversed_blocks))
            end
        end
    end
