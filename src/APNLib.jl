module APNLib

using Nemo

include("core/finite_field.jl")
include("core/matrix.jl")
include("core/nemo_compat.jl")
include("core/rcf.jl")
include("apn/tuples.jl")
include("apn/export.jl")
include("apn/fileUtils.jl")
include("apn/apn.jl")
include("apn/search.jl")

export gf2

export int_to_bits,
       int_to_column_vector,
       column_vector_to_int,
       each_column_vector

export check_square,
       check_same_size,
       check_same_field,
       check_compatible_pair

export matrix_multiplicative_order,
       matrix_is_similar

export check_order_space,
       proposition4_filter,
       proposition5_filter,
       standard_visit_order,
       offset_visit_order,
       c_reference_visit_order,
       addDDTInformation,
       removeDDTInformation,
       isComplete,
       nextFreePosition,
       nextVal,
       APNSearchContext,
       APNSearch,
       APNsearch,
       APNSearchClasses,
       APNsearchClasses,
       int_to_field_element,
       interpolate_sbox_polynomial,
       format_sbox_polynomial,
       is_apn,
       matrix_to_sbox,
       permutation_cycle_structure,
       matrix_cycle_structure,
       same_cycle_structure,
       is_permutation_tuple,
       is_power_similar,
       is_extended_power_similar,
       gen_permutation_tuples

export blocks_for_rcf,
       get_rcfs

export generate_tuples_file,
       generate_tuple_constants_file,
       generate_tuple_constants_files,
       generate_tuple_matrix_constants_file,
       generate_tuple_matrix_constants_files,
       extract_matrices,
       extract_matrices_from_tuple_lut,
       load_precomputed_tuple_constants,
       load_precomputed_tuple_matrix_constants,
       all_precomputed_tuple_class_indices,
       parse_class_indices_argument,
       normalize_precomputed_tuple_classes,
       precomputed_tuple_matrices,
       precomputed_tuple_row,
       precomputed_tuple_sboxes,
       DEFAULT_MONOMIAL_REPRESENTATIVE_DIMENSION,
       N7_MONOMIAL_REPRESENTATIVE_EXPONENTS,
       candidate_exponents_for_dimension,
       search_classes_from_saved_file,
       precomputed_gf2_matrix_pair,
       is_admissible_monomial_for_class,
       find_admissible_monomials_for_class,
       final_monomial_representatives_for_class,
       expected_monomial_representatives_for_class,
       format_monomial_representatives,
       print_admissible_monomials_for_class,
       find_monomial_representative_for_class,
       representative_selection_for_class,
       normalize_classes_to_compute,
       compute_monomial_representatives,
       compute_expected_monomial_representatives,
       parse_classes_argument,
       print_monomial_representatives,
       run_monomial_representative_cli,
       save_search_result_constant,
       search_result_constant_name,
       tuple_to_sbox_row,
       tuples_to_sbox_rows

end
