...
function is_square(A::FqMatrix)
    return nrows(A) == ncols(A)
end

function check_square(A::FqMatrix; name::String = "Matrix")
    is_square(A) || error("$name must be square")
end

function check_same_size(A::FqMatrix, B::FqMatrix)
...
function check_compatible_pair(A::FqMatrix, B::FqMatrix)
...
function matrix_power(A::FqMatrix, i::Int)
    i >= 0 || error("Exponent must be non-negative")
    return A^i
end